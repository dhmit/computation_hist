...
def populate_from_metadata(file_name=None):
    ...
    cursor.execute('''CREATE VIRTUAL TABLE doc_fts USING FTS4(id, title, text, author, recipient, 
                                                                                       cced);''')
    for d in Document.objects.all():
        ...
        # Technically, sqlite expects quotation marks and apostrophes to be escaped by doubling
        ...
        cursor.execute(f'''INSERT INTO doc_fts(id, title, text, author, recipient, cced) 
                                      VALUES("{d.pk}", "{title}", "{text}", "{author}", 
                                             "{recipient}", "{cced}");''')
